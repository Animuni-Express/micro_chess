...
import { multiplayer } from './onlineTransport';
...
// Promise chains, not async/await: the Kindle bundle targets ES5 and esbuild
// cannot lower the generators that async functions compile to.
export function KindleOnlineGameApp() {
  ...
  const canMove = game && game.status === 'active' && game.turn === playerColor && !pendingMove;

  const refreshGame = useCallback(
    (id) => {
      const gid = id || gameId;
      if (!gid) return;
      multiplayer('get_game', { gameId: gid })
        .then((response) => {
  ...
  const openGame = useCallback((id, response) => {
    setGameId(id);
    setGame((response && response.game) || null);
    setPlayerColor((response && response.playerColor) || null);
    setGameUrl(id);
    setMessage('');
  }, []);
  ...
  useEffect(() => {
    window.render_game_to_text = () =>
      JSON.stringify({
        ...
        timeControl: (game && game.time_control) || timeControl,
        connection,
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      multiplayer('matchmaking_status')
        .then((response) => {
          if (response.gameId) {
            setQueueing(false);
  ...
  const createRoom = useCallback(() => {
    ...
    multiplayer('create_room', { timeControl })
      .then((response) => openGame(response.game.id, response))
      .catch((error) => setMessage(error.message));
  ...
  const joinRoom = useCallback(() => {
    ...
    multiplayer('join_room', { roomCode: roomCode.trim().toUpperCase() })
      .then((response) => openGame(response.game.id, response))
      .catch((error) => setMessage(error.message));
  }, [roomCode, openGame]);
  ...
  const joinMatchmaking = useCallback(() => {
    ...
    multiplayer('join_matchmaking', { timeControl })
      .then((response) => {
  ...
  const leaveMatchmaking = useCallback(() => {
    ...
    multiplayer('leave_matchmaking').then(finish, finish);
  }, []);
  ...
  const onSquareClick = useCallback(
    (square) => {
      ...
      if (!selectedSquare) {
        if (clicked && clicked.color === playerColor) setSelectedSquare(square);
        return;
      }
      if (square === selectedSquare) {
        setSelectedSquare(null);
      } else if (clicked && clicked.color === playerColor) {
        setSelectedSquare(square);
      } else {
        const fromPiece = chess.get(selectedSquare);
        const promotion = fromPiece && fromPiece.type === 'p' && (square[1] === '1' || square[1] === '8');
  ...
  const gameAction = useCallback(
    (action) => {
      if (!game) return;
      multiplayer(action, { gameId: game.id })
        .then((response) => {
      ...
      <div className="setup-view online-lobby">
        ...
        <div className="setup-option">
          ...
          <select
            ...
            value={timeControl}
            onChange={(event) => setTimeControl(Number(event.currentTarget.value))}
          >
            {TIME_CONTROLS.map((control) => (
              <option value={control.value} key={control.value}>
                {control.label}
              </option>
            ))}
          </select>
        ...
        <div className="room-join">
          <input
            ...
            maxLength="6"
            value={roomCode}
            onInput={(event) => setRoomCode(event.currentTarget.value.toUpperCase())}
        ...
        {queueing ? (
          <div className="queue-status">
            <p>Finding a {timeControl / 60}-minute opponent…</p>
            <button className="btn btn-secondary" onClick={leaveMatchmaking}>
              CANCEL
            </button>
          </div>
        ) : (
          <button className="btn online-match-button" onClick={joinMatchmaking}>
            FIND A MATCH
          </button>
        )}
        {message && (
          <p className="online-message" role="status">
            {message}
          </p>
        )}
      </div>
    ...
    <div className="container online-game">
      <div className="header">
        ...
        <div className="online-meta">
          ...
          <span>{game && game.mode === 'room' ? 'Room ' + game.room_code : (game ? game.time_control : 300) / 60 + ' min match'}</span>
        </div>
      </div>
      {game && game.mode === 'room' && game.status === 'waiting' && (
      ...
      <Clock label={playerColor === 'b' ? 'You (Black)' : 'Opponent (Black)'} value={displayedClock('b')} active={game && game.turn === 'b'} />
      <div className="board-container">
        ...
        <PromotionDialog
          ...
          color={(pendingPromotion && pendingPromotion.color) || 'w'}
          onChoose={(piece) => submitMove(pendingPromotion.from, pendingPromotion.to, piece)}
        />
      </div>
      <Clock label={playerColor === 'w' ? 'You (White)' : 'Opponent (White)'} value={displayedClock('w')} active={game && game.turn === 'w'} />
      <div id="status" className={pendingMove ? 'thinking' : ''}>
        {pendingMove ? 'Sending move…' : statusText(game, playerColor)}
      </div>
      {isDrawOffer && (
        <div className="draw-offer">
          Opponent offered a draw.
          <button className="btn" onClick={() => gameAction('accept_draw')}>
            ACCEPT DRAW
          </button>
          <button className="btn btn-secondary" onClick={() => gameAction('decline_draw')}>
            DECLINE
          </button>
        </div>
      )}
      {game && game.status === 'active' && (
        <div className="controls">
          <button className="btn btn-secondary" onClick={() => gameAction('offer_draw')}>
            OFFER DRAW
          </button>
          <button className="btn" onClick={() => gameAction('resign')}>
            RESIGN
          </button>
        </div>
      )}
